...
GC1000_DRVSRC:= modules/graphics/galcore_src
...
TELEPHONY_DRVSRC:= modules/marvell-telephony/drivers
.PHONY: telephony_modules clean_telephony_modules
telephony_modules:
	$(log) "making telephony drivers..."
	$(hide)cd $(TELEPHONY_DRVSRC) && \
	make -j1 TARGET_DEVICE=$(TARGET_DEVICE) KERNEL_OUTDIR=$(shell pwd)/$(KERNEL_OUTDIR) hide=$(hide)
	$(hide)mkdir -p $(KERNEL_OUTDIR)/modules/
	$(log) "telephony driver done."

clean_telephony_modules:
	$(hide)cd $(TELEPHONY_DRVSRC) &&\
	make clean TARGET_DEVICE=$(TARGET_DEVICE) KERNEL_OUTDIR=$(shell pwd)/$(KERNEL_OUTDIR) hide=$(hide)
	$(log) "telephony driver cleaned."

GEU_DRVSRC:= modules/security/wtpsp/drv/src
.PHONY: geu clean_geu
geu:
	$(log) "making security driver..."
	$(hide)cd $(GEU_DRVSRC) && \
	make KDIR=$(KERNELDIR) ARCH=arm CROSS_COMPILE:=$(KERNEL_TOOLCHAIN_PREFIX) M=$(PWD)
	$(hide)mkdir -p $(KERNEL_OUTDIR)/modules/
